...
identical(rownames(traits_final), colnames(matrix_ordered))
...
#get different species year con 


species_year18<- data.frame(species=old_new2$species, year=old_new2$year, site=old_new2$site)
torm<- which(species_year18$year==2019)
species_year18<-species_year18[-c(torm),]

#torm<- which (species_year18$site== '1' | species_year18$site== '4'| species_year18$site== '6'| species_year18$site== '8' |
               # species_year18$site== '15' | species_year18$site== '21' | species_year18$site== '22' | species_year18$site== '24' |
               # species_year18$site== '28')
#
#species_year18<-species_year18[-c(torm),]



unique(species_year18$year)

species_year18<-unique(species_year18)

species_year_con18<- species_year18 %>% group_by(species) %>% summarise(yearcount= mean(year))

species_year_con18$yearcount<-as.character(species_year_con18$yearcount)

species_year_con18$yearcount[species_year_con18$yearcount==1976]<-'old'
species_year_con18$yearcount[species_year_con18$yearcount==2018]<-'new'
species_year_con18$yearcount[species_year_con18$yearcount==1997]<-'both'



#summary table for species-year
both<-which(species_year_con18$yearcount=='both')
both_fish<-species_year_con18[both,]

fish_2018<- which(species_year_con18$yearcount=='new')
fish_new<-species_year_con18[fish_2018,]

fish_1975<-which(species_year_con18$yearcount=='old')
fish_old<-species_year_con18[fish_1975,]

write.csv(both_fish, 'both_years_fish_sp.csv')
write.csv(fish_new, 'fish_sp_2018_only.csv')
write.csv(fish_old, 'fish_sp_1975_only.csv')


#ok now rarefaction curves
#rarefaction curves----
old_rare<- old_new2 %>% filter( year==1976)
new_rare<-old_new2 %>% filter( year==2018)

length(unique(old_rare$species))
length(unique(new_rare$species))

old_rare<- old_rare[,c(2,3,5)]
new_rare<- new_rare[,c(2,3,5)]

duplicated(old_rare)

old_rare<- acast(old_rare, site~species)
new_rare<- acast(new_rare, site~species)

#remove 1976 sites with no current data
torm<-which(rownames(old_rare)== '1' |rownames(old_rare)== '4' |rownames(old_rare)== '6' |rownames(old_rare)== '8' |rownames(old_rare)== '15' |
              rownames(old_rare)== '18' |rownames(old_rare)== '21' |rownames(old_rare)== '22' |rownames(old_rare)== '24' |rownames(old_rare)== '28' )

old_rare<-old_rare[-c(torm),]

colSums(old_rare)

length(which(colSums(old_rare)<1))
length(which(colSums(new_rare)<1))

#remove colsums=0
#torm<-which(colSums(old_rare)<1)

#old_rare<-old_rare[,-c(torm)]

#now make spec acum curves
par(mfrow=c(1,2))
sp1<-specaccum(old_rare, method='random', permutations = 100)
...
sp2<-specaccum(new_rare, method='random', permutations = 100)
